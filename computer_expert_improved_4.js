...
function ConvertSecondsToTimeObj(seconds) {
  this.hours = Math.trunc(seconds / 3600);
  this.minutes = Math.trunc((seconds - (Math.trunc(seconds / 3600)) * 3600) / 60);
  this.seconds = seconds - Math.trunc(seconds / 3600) * 3600 - Math.trunc((seconds - (Math.trunc(seconds / 3600)) * 3600) / 60) * 60;
}

function ConvertArrToTimeObj(hours, minutes, seconds) {
...
function parseTeamRawData(stringRawDataRunners) {
  return stringRawDataRunners.split(', ')
    .map(item => item.split('|'))
    .map(item => new ConvertArrToTimeObj(item[0], item[1], item[2]));
}
...
function formatAnalyticsString(teamAnalytics) {
  return 'Range: ' + formatTimeString(teamAnalytics.range) + ' Average: ' + formatTimeString(teamAnalytics.average) + ' Median: ' + formatTimeString(teamAnalytics.median);
}
