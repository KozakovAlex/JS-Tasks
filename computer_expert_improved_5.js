...
function convertTimeObjToSeconds(runnersResult) {
  return runnersResult.hours * 3600 + runnersResult.minutes * 60 + runnersResult.seconds;
}
...
function calculateTeamAnalytics(runnersResults) {
  let arrayInSecSort = runnersResults.map(item => convertTimeObjToSeconds(item)).sort();
